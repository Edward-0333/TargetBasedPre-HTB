...
class LocalEncoder(nn.Module):
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        for t in range(self.historical_steps):
            aaa = data['padding_mask'][:, t].cpu().numpy()
            data[f'edge_index_{t}'], _ = subgraph(subset=~data['padding_mask'][:, t], edge_index=data.edge_index)
            ccc = data.edge_index.cpu().numpy()
            bbb = data[f'edge_index_{t}'].cpu().numpy()
            data[f'edge_attr_{t}'] = \
...
class AAEncoder(MessagePassing):
    ...
    def message(self,
        ...
        test_alpha = alpha.cpu().detach().numpy()

        alpha = softmax(alpha, index, ptr, size_i)
        alpha = self.attn_drop(alpha)
        # aaa = value * alpha.unsqueeze(-1)
    ...
    def _mha_block(self,
        ...
        temp = self.propagate(edge_index=edge_index, x=x, center_embed=center_embed,
                                                    edge_attr=edge_attr, rotate_mat=rotate_mat, size=size)
        center_embed = self.out_proj(temp)
